...
class ParamsPoissonRegressionHMMEmissions(NamedTuple):
...
class ParamsPoissonRegressionHMM(NamedTuple):
    initial: ParamsStandardHMMInitialState
    transitions: ParamsStandardHMMTransitions
    emissions: ParamsPoissonRegressionHMMEmissions


class PoissonRegressionHMMEmissions(HMMEmissions):
    def __init__(self,
                 num_states,
                 input_dim,
                 emission_dim):
    ...
    def distribution(self, params, state, inputs):
        prediction = jnp.exp(params.weights[state] @ inputs)
        # prediction +=  params.biases[state]
        return tfd.Poisson(prediction)
...
class PoissonRegressionHMM(HMM):
    ...
    *Note: in the future we add a* matrix-normal-inverse-Wishart_ *prior (see pg 576).*
    ...
    def __init__(self,
        ...
        emission_component = PoissonRegressionHMMEmissions(num_states, input_dim, emission_dim)
    ...
    def initialize(self,
        ...
        return ParamsPoissonRegressionHMM(**params), ParamsPoissonRegressionHMM(**props)